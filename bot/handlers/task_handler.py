...
async def add_task_command(message: Message, state: FSMContext):
    ...
    """
    await send_message_with_keyboard(message, Messages.WAITING_INPUT.value)
...
@router.message(
    TaskCreationStates.WAITING_CONFIRMATION_TEXT, F.text.in_(Answers.YES.value)
)
...
@router.message(
    TaskCreationStates.WAITING_CONFIRMATION_TEXT, F.text.in_(Answers.NO.value)
)
...
async def list_tasks_command(message: Message):
    await send_message_with_keyboard(
        message, Messages.IN_DEVELOPMENT.value, reply_kb.cancel_btn
    )
